...
class KotlinTextDocumentService(
    ...
    private val definitionHandler = DefinitionHandler(uriContentProvider.classContentProvider, tempDirectory, config.externalSources, cp)
    ...
    var lintRecompilationCallback: () -> Unit
        ...
        set(callback) { sourcePath.beforeCompileCallback = callback }
    ...
    private fun recover(position: TextDocumentPositionParams, recompile: Recompile = Recompile.NEVER): Pair<CompiledFile, Int>? {
    ...
    override fun codeAction(params: CodeActionParams): CompletableFuture<List<Either<Command, CodeAction>>> = asyncExecutor.compute {
    ...
    override fun inlayHint(params: InlayHintParams): CompletableFuture<List<InlayHint>> = asyncExecutor.compute {
    ...
    override fun hover(position: HoverParams): CompletableFuture<Hover?> = asyncExecutor.compute {
        reportTime {
    ...
    override fun documentHighlight(position: DocumentHighlightParams): CompletableFuture<List<DocumentHighlight>> = asyncExecutor.compute {
    ...
    override fun definition(position: DefinitionParams): CompletableFuture<Either<List<Location>, List<LocationLink>>> = asyncExecutor.compute {
        reportTime {
            LOG.info("Go-to-definition at {}", describePosition(position))

            val (file, cursor) = recover(position) ?: return@compute Either.forLeft(emptyList())
            definitionHandler.goToDefinition(file, cursor)
                ?.let(::listOf)
    ...
    override fun rangeFormatting(params: DocumentRangeFormattingParams): CompletableFuture<List<TextEdit>> = asyncExecutor.compute {
    ...
    override fun rename(params: RenameParams): CompletableFuture<WorkspaceEdit?> = asyncExecutor.compute {
        val (file, cursor) = recover(params, Recompile.NEVER) ?: return@compute null
        renameSymbol(file, cursor, sourcePath, params.newName)
    }

    override fun completion(position: CompletionParams): CompletableFuture<Either<List<CompletionItem>, CompletionList>> = asyncExecutor.compute {
        reportTime {
            LOG.info("Completing at {}", describePosition(position))

            val (file, cursor) = recover(position) ?: return@compute Either.forRight(CompletionList()) // TODO: Investigate when to recompile
    ...
    override fun documentSymbol(params: DocumentSymbolParams): CompletableFuture<List<Either<SymbolInformation, DocumentSymbol>>> = asyncExecutor.compute {
    ...
    override fun didSave(params: DidSaveTextDocumentParams) {
        ...
        val uri = parseURI(params.textDocument.uri)
        lintNow(uri)
        lintDebouncer.schedule {
            sourcePath.save(uri)
        }
    }

    override fun signatureHelp(position: SignatureHelpParams): CompletableFuture<SignatureHelp?> = asyncExecutor.compute {
        reportTime {
    ...
    override fun formatting(params: DocumentFormattingParams): CompletableFuture<List<TextEdit>> = asyncExecutor.compute {
    ...
    override fun references(position: ReferenceParams) = asyncExecutor.compute {
    ...
    override fun semanticTokensFull(params: SemanticTokensParams) = asyncExecutor.compute {
    ...
    override fun semanticTokensRange(params: SemanticTokensRangeParams) = asyncExecutor.compute {
